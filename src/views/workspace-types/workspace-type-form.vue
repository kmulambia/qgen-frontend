<template>
  <!-- Create/Edit Workspace Type Modal -->
  <div
    v-if="open"
      ...
      >
        <div v-if="hasField('name')">
          ...
          <input
            ...
            :class="[
              ...
              'theme-input w-full border px-3 py-2 focus:outline-none focus:ring-2 dark:text-white dark:border-gray-600 transition-all duration-200',
            ]"
          />
        ...
        <div v-if="hasField('description')">
          ...
          <textarea
            ...
            placeholder="Enter workspace type description (optional)"
            :class="[
              errors.description
                ? 'border-red-300 focus:ring-red-500 focus:border-red-500'
                : 'border-gray-300 focus:ring-primary-500 focus:border-primary-500',
...
import type { IWorkspaceType, IField } from '@/interfaces'
import { WorkspaceTypeSchema } from '@/schemas/workspace-type-schemas'
...
const canCreate = computed(
  () =>
    ...
    sessionStore.hasPermission('configurations.workspace-type.*') ||
    sessionStore.hasPermission('configurations.workspace-type.create'),
)
...
const canEdit = computed(
  () =>
    ...
    sessionStore.hasPermission('configurations.*') ||
    sessionStore.hasPermission('configurations.workspace-type.*') ||
    sessionStore.hasPermission('configurations.workspace-type.update'),
)
...
const canView = computed(
  () =>
    ...
    sessionStore.hasPermission('configurations.*') ||
    sessionStore.hasPermission('configurations.workspace-type.*') ||
    sessionStore.hasPermission('configurations.workspace-type.view'),
)
...
const props = defineProps({
  ...
  workspaceType: { type: Object as PropType<IWorkspaceType | null>, default: () => null },
  fields: {
  ...
  title: { type: String, default: 'Create Workspace Type' },
  readonly: { type: Boolean, default: false },
  mode: { type: String as PropType<'create' | 'edit' | 'view'>, default: 'create' },
})

const { loading, open, workspaceType, title, readonly, mode } = toRefs(props)
...
const { handleSubmit, errors, resetForm } = useForm<IWorkspaceType>({
  validationSchema: WorkspaceTypeSchema(t),
  initialValues: workspaceType.value || { name: '', description: '', is_system_defined: false },
})
