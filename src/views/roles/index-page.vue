...
<template>
  ...
  <div v-else class="min-h-screen bg-gray-50 dark:bg-gray-900">
    ...
    <RoleForm
      :open="showModal"
      :key="showModal+modalMode+selectedRole?.id+Date.now()"
    ...
    <ConfirmationComponent
      ...
      :title="`Delete Role`"
      :description="`Are you sure you want to ${hardDeleteMode ? 'permanently' : 'soft'} delete '${roleToDelete?.name}'? ${hardDeleteMode ? 'This action cannot be undone.' : 'You can restore it later.'}`"
      :secure-mode="true"
...
import type { IRole, IRequestFilterCondition } from '@/interfaces'
...
import RoleForm from '@/views/roles/role-form.vue'
...
const canDelete = computed(() =>
  sessionStore.hasPermission('*') ||
  sessionStore.hasPermission('role.*') ||
  sessionStore.hasPermission('role.delete')
)
...
const canList = computed(() =>
  sessionStore.hasPermission('*') ||
  sessionStore.hasPermission('configurations.*') ||
  sessionStore.hasPermission('configurations.role.*') ||
  sessionStore.hasPermission('configurations.role.list')
)
...
const modalTitle = ref('Create Role')
...
const columns = [
  ...
  },
  {
    ...
    exportable: false,
    render: (data: IRole & { name: string }) => {
      // Filter actions based on system defined status
      let availableActions = [...dataTableActions.value]

      // For system-defined roles, only allow view and manage actions
      if (data.is_system_defined) {
        availableActions = availableActions.filter(action => action === 'view' || action === 'manage')
      }
...
const tableOptions: any = {
  ...
  buttons: [
    ...
    },
    {
      ...
      filename: () => `roles_${moment().format('YYYY-MM-DD_HH-mm')}`,
    },
    {
      extend: 'print',
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ajax: async (dtParams: any, callback: (response: IDataTableCallback<IRole>) => void) => {
...
const handleDeleteConfirm = async () => {
  ...
  try {
    await roleStore.delete(roleToDelete.value.id!, hardDeleteMode.value)
    if (table.value?.dt) {
      table.value.dt.ajax.reload()
    }
    toast.success(`Role ${hardDeleteMode.value ? 'permanently deleted' : 'soft deleted'} successfully`)
  } catch (error) {
...
const handleRoleSubmit = async (roleData: IRole) => {
  ...
  if (selectedRole.value && selectedRole.value.id) {
    ...
    // Check if trying to edit a system-defined role
    if (selectedRole.value.is_system_defined) {
      toast.error('Cannot edit system-defined roles')
      return
    }
  } else {
    // Create new role
  ...
  try {
    if (selectedRole.value && selectedRole.value.id) {
      // Update existing role
      await roleStore.update(selectedRole.value.id, roleData)
      toast.success('Role updated successfully')
    } else {
      // Create new role
      await roleStore.create(roleData)
...
const handleFetch = async (dtParams: DataTableParams): Promise<IDataTableCallback<IRole>> => {
  try {
    if (!canList.value) {
    ...
    const response = await roleStore.query(query, filters)
...
const setPermissions = () => {
   ...
   */
  //view permission
  if (sessionStore.hasAnyPermission(['.*', 'role.*', 'role.view'])) {
    dataTableActions.value.push('view')
  }

  // edit permission
  if (sessionStore.hasAnyPermission(['.*', 'role.*', 'role.update'])) {
    dataTableActions.value.push('edit')
  }

  //manage permission
  if (sessionStore.hasAnyPermission(['.*', 'role.*', 'role.manage'])) {
    dataTableActions.value.push('manage')
  }

  //delete permission
  if (sessionStore.hasAnyPermission(['.*', 'role.*', 'role.delete'])) {
...
const setupEventHandlers = () => {
  ...
  setupEventHandler('manage', (row) => {
    ...
    router.push(`/${roleName}/user-management/roles/manage-permissions/${row.id}`)
  })
  ...
  setupEventHandler('delete', (row) => {
    ...
    if (row.is_system_defined) {
      toast.error('Cannot delete system-defined roles')
      return
    }
